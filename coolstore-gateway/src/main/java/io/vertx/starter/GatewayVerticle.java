  ...
  public void start() {
    ...
    retriever.getConfig(ar -> {
      if(ar.failed()){
        ...
        Integer catalogApiPort = config.getInteger("COMPONENT_CATALOG_PORT", 8085);
        ...
        LOGGER.info("Catalog Service Endpoint:" + catalogApiHost + ":" + catalogApiPort.toString());
        ...
        LOGGER.info("Inventory Service Endpoint: " +inventoryApiHost +":"+ inventoryApiPort.toString());

        vertx.createHttpServer()
          .requestHandler(router).listen(Integer.getInteger("http.port", 8090));

        LOGGER.info("Server is running on port " + Integer.getInteger("http.port", 8090));
      }
    });
  }
    ...
    return inventory
      ...
      .map(resp -> {
        if (resp.statusCode() != 200) {
          LOGGER.warn("Inventory error for {}: status code {}",
            product.getString("itemId"), resp.statusCode());
          return product.copy();
        }
        return product.copy().put("availability",
          new JsonObject().put("quantity", resp.body().getValue("quantity")));
      });
  }
